...
export function convertFileToJSON(fileData, settings, ndjson = false) {

    let convertedData = ndjson ? "" : [];
    ...
    fileData?.forEach((row) => {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            switch (types[i]) {
                ...
                case "slug":
                    ...
                    break;
                case "url":
                    convertedRow[headers[i]] = convertedStringToURL(values[i]);
                    break;
                default:
                    break;
            }
        ...
        if (ndjson) {
            convertedData += (JSON.stringify(convertedRow) + "\n");
        } else {
...
export function convertedStringToURL(string) {
